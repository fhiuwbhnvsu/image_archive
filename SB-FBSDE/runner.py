...
class Runner():
    ...
    def dsm_train(self, opt, num_itr, batch_x, batch_t):
        ...
        #p = data.my_build_data_sampler(opt, batch_x)
    ...
    def dsm_train_first_stage(self, opt):
        ...
        #and self.z_f.zero_out_last_layer
        self.dsm_train(
    ...
    def sb_alternate_train(self, opt):
        ...
        time_path = str(opt.ckpt_path) + "time.npz"
        np.savez(time_path, history=np.array(times), total=np.array([sum(times)]))
        print(f"Training time: {sum(times)}")
    ...
    def compute_EMD(self):
        ...
        emds_L2 = []
        for _ in range(100):
            _, _, x_term = self.dyn.sample_traj(self.ts, z)
            ref = self.q.sample()
            sz = min(len(x_term), len(ref))
            emd_L1 = util.compute_emd2(x_term[:sz], ref[:sz], p=1)
            emd_L2 = util.compute_emd2(x_term[:sz], ref[:sz], p=2)
            emds_L1.append(emd_L1)
            emds_L2.append(emd_L2)
        

        print(f"[TEST] forward L1, {np.mean(emds_L1)} ± {np.std(emds_L1)}")
        print(f"[TEST] forward L2, {np.mean(emds_L2)} ± {np.std(emds_L2)}")

    
    @torch.no_grad()
    def compute_CDD(self, opt):
        z = self.z_f # lmt
        z = freeze_policy(z)
        
        z_fh, _, dyn_fh = util.restore_policy_and_dyn(opt, self, opt.load_fh)
        z_lh, _, dyn_lh = util.restore_policy_and_dyn(opt, self, opt.load_lh)
        z_fh = freeze_policy(z_fh)
        z_lh = freeze_policy(z_lh)
        x = self.p.sample()

        opt_ = copy.deepcopy(opt)
        opt_.T = opt.t0 + 1.0
        ds, _ = data.build_boundary_distribution(opt_)
        param = ds.scaler_params()
        param_lmt = self.p.scaler_params()
        
        # normlize
        x_all = ((x * param_lmt['scale']) + param_lmt['mean'] - param['mean']) / param['scale']
        p = 2
        num_repeat = 100

        
        #th = (opt.t0 + opt.T) / 2
        ts = torch.linspace(opt.t0, opt.T, opt.interval+1)
        eval_idx = torch.linspace(0, len(ts)-1, 7).long()
        ts_fh = ts[:len(ts)//2]
        ts_lh = ts[len(ts)//2:]

        xs, _, _ = self.dyn.sample_traj_unc(ts, z, num_repeat=num_repeat, x=x)
        
        xs_fh, _, x_term = dyn_fh.sample_traj_unc(ts_fh, z_fh, num_repeat=num_repeat, x=x_all)
        
        xs_lh, _, _ = dyn_lh.sample_traj_unc(ts_lh, z_lh, num_repeat=num_repeat, x=x_term[:, 0, :])
        xs_all = torch.cat([xs_fh, xs_lh], dim=1)
        
        data_size, t_size = xs.size(0), xs.size(1)
        # denormalize
        scale_lmt = param_lmt['scale'].repeat(data_size, t_size, num_repeat, 1).to(xs)
        mean_lmt = param_lmt['mean'].repeat(data_size, t_size, num_repeat, 1).to(xs)
        scale = param['scale'].repeat(data_size, t_size, num_repeat, 1).to(xs)
        mean = param['mean'].repeat(data_size, t_size, num_repeat, 1).to(xs)
        xs_all = (((xs_all * scale) + mean - mean_lmt) / scale_lmt)

        xs = xs[:, eval_idx, :]
        xs_all = xs_all[:, eval_idx, :]
        t_size = len(eval_idx)
        res = {}
        for j in range(1, t_size):
            losses = []
            for i in range(data_size):
                M = torch.pow(torch.cdist(xs[i, j], xs_all[i, j], p=p),p)
                a, b = ot.unif(num_repeat), ot.unif(num_repeat)
                loss = ot.emd2(a, b, M.cpu().detach().numpy())
                if p == 2:
                    loss = np.sqrt(loss)
                losses.append(loss)
            res[f't={self.ts[j].item()}'] = sum(losses) / data_size
