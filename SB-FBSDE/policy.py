...
def build(opt, dyn, direction):
    ...
    use_t_idx = (net_name in ['toy', 'Unet', 'DGLSB', 'OTFlow']) # t_idx is handled internally in ncsnpp
...
def _build_net(opt, net_name):
    ...
    elif net_name == 'OTFlow':
        from models.OTFlow.ot_flow import build_otflow
        net = build_otflow(opt.data_dim[0])
    else:
        raise RuntimeError()
    return net
