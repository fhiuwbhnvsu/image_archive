...
class BaseSDE(metaclass=abc.ABCMeta):
    ...
    def sample_traj(self, ts, policy, x=None, corrector=None, apply_trick=True, save_traj=True):
    ...
    def sample_traj_unc(self, ts, policy, num_repeat, x=None, corrector=None, apply_trick=True, save_traj=True):
        Xs, Zs, Xterms = [], [], []
        for _ in range(num_repeat):
            xs, zs, x_term = self.sample_traj(ts, policy, x=x, corrector=corrector, apply_trick=apply_trick, save_traj=save_traj)
            Xs.append(xs.unsqueeze(2))
            Zs.append(zs.unsqueeze(2))
            Xterms.append(x_term.unsqueeze(1))
        
        Xs = torch.cat(Xs, dim=2)
        Zs = torch.cat(Zs, dim=2)
        Xterms = torch.cat(Xterms, dim=1)
        return Xs, Zs, Xterms
